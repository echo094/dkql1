...
!(async () => {
  if (!cookiesArr[0]) {
    ...
    let activityList = [
        {'name':'鞋靴馆','end':'1688054400000','url':'https://prodev.m.jd.com/mall/active/mn56EvrbpDFT1QbwFD1FNuxqYpV/index.html'},//2023-02-21---2023-06-30
    ];
    $.info = {};
...
async function signRun(actInfo) {
    ...
    mch = thisHtml.match(/window.__api_data__ = ([^\n]+);\n/)
    if (mch) {
        list = JSON.parse(mch[1])?.floorList
        for (obj of list) {
            str = obj?.boardParams?.interaction
            if (!str) {
                continue
            }
            encryptProjectId = str.match(/"encryptProjectId":"(.*?)"/)
            encryptAssigmentId = str.match(/"encryptAssignmentId":"(.*?)"/)
            if (!encryptProjectId || !encryptAssigmentId) {
                continue
            }
            actInfo.encryptProjectId = encryptProjectId[1]
            actInfo.encryptAssigmentIds = encryptAssigmentId[1]
            actInfo.activity_id = actInfo.url.match(/active\/(.*?)\/index/)[1]
            actInfo.template_id = obj.styleId
            actInfo.floor_id = obj.id
            actInfo.enc = obj.enc
            break
        }
    }
    ...
    for (let i = 0; i < assignmentList.length; i++) {
        ...
        if(oneInfo.completionFlag){
            ...
            let info = {
                ...
                "extParam": {
                    "forceBot": "1",
                    "sceneid": "babel_".concat(actInfo.activity_id)
                },
                "activity_id": actInfo.activity_id,
                "template_id": actInfo.template_id,
                "floor_id": actInfo.floor_id,
                "enc": actInfo.enc
            }
